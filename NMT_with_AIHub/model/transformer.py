...
class MultiHeadAttention_esb(nn.Module):
  def __init__(self, head_num =8 , d_model = 512,dropout = 0.1):
    super(MultiHeadAttention_esb,self).__init__()
  ...
  def forward(self, query, key, value, mask = None):
    ...
    return self.w_o(attention_result), attention_result, attention_score
...
# attention ensemble
class Decoder_esb(nn.Module):
  def __init__(self, d_model,head_num, dropout):
    super(Decoder_esb,self).__init__()
    self.masked_multi_head_attention = MultiHeadAttention_esb(d_model= d_model, head_num= head_num)
    self.residual_1 = ResidualConnection(d_model,dropout=dropout)

    self.encoder_decoder_attention = MultiHeadAttention_esb(d_model= d_model, head_num= head_num)
    self.residual_2 = ResidualConnection(d_model,dropout=dropout)
  ...
  def forward(self, target, encoder_output, target_mask, encoder_mask):
    # target, x, target_mask, input_mask
    # encoder_output에 model.encode 값이 들어감.
    # x = target
    # x, y, z = self.masked_multi_head_attention(x, x, x, target_mask)
    # x = self.residual_1(target, x)
    # e_x, e_y, e_z = self.encoder_decoder_attention(x, encoder_output, encoder_output, encoder_mask)
    # x = self.residual_2(x, e_x)
    # x = self.residual_3(x, self.feed_forward)
    x = self.residual_1(target, lambda x: self.masked_multi_head_attention(x, x, x, target_mask))
    x = self.residual_2(x, lambda x: self.encoder_decoder_attention(x, encoder_output, encoder_output, encoder_mask))
    x = self.residual_3(x, self.feed_forward)
...
class Transformer(nn.Module):
  def __init__(self,vocab_num, d_model, max_seq_len, head_num, dropout, N):
    ...
    # self.decoders = clones(Decoder_esb(d_model=d_model, head_num=head_num, dropout=dropout), N)
